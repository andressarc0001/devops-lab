...
exports.enableStateDataCookie = enableStateDataCookie;
exports.getIsStateDataCookieEnabled = getIsStateDataCookieEnabled;
exports.setStateDataCookieIfEnabled = setStateDataCookieIfEnabled;
exports.clearStateDataCookie = clearStateDataCookie;
exports.getStateDataCookies = getStateDataCookies;
const assert_1 = require("../tools/tsafe/assert");
const id_1 = require("../tools/tsafe/id");
const SEPARATOR = "%_";
function serializeStateDataCookie(params) {
    const { stateDataCookie, time } = params;
    let cookieValue = `${Math.floor(time / 1000)}`;
    cookieValue += `${SEPARATOR}${encodeURIComponent(stateDataCookie.rootRelativeRedirectUrl)}`;
    switch (stateDataCookie.action) {
        case "login":
            cookieValue += `${SEPARATOR}${encodeURIComponent(stateDataCookie.rootRelativeRedirectUrl_consentRequiredCase)}`;
            break;
        case "logout":
            break;
        default:
            (0, assert_1.assert)(false, "30449274");
    }
    return cookieValue;
}
function parseStateDataCookie(params) {
    ...
    const rootRelativeRedirectUrl_consentRequiredCase = rest.length === 1 ? undefined : decodeURIComponent(rest[1]);
    ...
    const stateDataCookie = rootRelativeRedirectUrl_consentRequiredCase === undefined
        ? (0, id_1.id)({
            action: "logout",
            rootRelativeRedirectUrl
        })
...
let isEnabled = false;
function enableStateDataCookie() {
    isEnabled = true;
}
function getIsStateDataCookieEnabled() {
    return isEnabled;
}
const COOKIE_NAME_PREFIX = "oidc_";
function setStateDataCookieIfEnabled(params) {
...
//# sourceMappingURL=StateDataCookie.js.map