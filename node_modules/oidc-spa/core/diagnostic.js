...
exports.createInvalidClientCredentialsInitializationError = createInvalidClientCredentialsInitializationError;
...
const wildcardsMatch_1 = require("../tools/wildcardsMatch");
async function createWellKnownOidcConfigurationEndpointUnreachableInitializationError(params) {
    ...
    const commonFallbackMessagePart = [
        ...
        `Endpoint that couldn't be reached: ${issuerUri}${OidcMetadata_1.WELL_KNOWN_PATH}`
    ].join("\n");
    if (!(0, keycloak_1.isKeycloak)({ issuerUri })) {
        return new OidcInitializationError_1.OidcInitializationError({
            messageOrCause: [
    ...
    if (keycloakUtils.issuerUriParsed.kcHttpRelativePath === undefined) {
        ...
        const isValid = await (0, getIsValidRemoteJson_1.getIsValidRemoteJson)(`${issuerUri_candidate}${OidcMetadata_1.WELL_KNOWN_PATH}`);
        if (isValid) {
            return new OidcInitializationError_1.OidcInitializationError({
                messageOrCause: [
...
async function createIframeTimeoutInitializationError(params) {
    ...
    check_if_well_known_endpoint_is_reachable: {
        const isValid = await (0, getIsValidRemoteJson_1.getIsValidRemoteJson)(`${issuerUri}${OidcMetadata_1.WELL_KNOWN_PATH}`);
        if (isValid) {
    ...
    // Investigate if framing was prevented by some header defined policies
    {
        ...
        content_security_policy_issue: {
            ...
            const csp_parsed = Object.fromEntries(cspHeaderValue
                .split(";")
                .filter(part => part !== "")
                .map(statement => {
                const [directive, ...values] = statement.split(" ");
                (0, assert_1.assert)(directive !== undefined);
                (0, assert_1.assert)(values.length !== 0);
            ...
            frame_src_issue: {
                ...
                const hasIssue = (() => {
                    for (const frameSrcValue of frameSrcValues) {
                        ...
                        if ((0, wildcardsMatch_1.getDoMatchWildcardsPattern)({
                ...
                const recommendedValue = (() => {
                    ...
                    const { hostname: hostname_authorizationEndpoint, origin: origin_authorizationEndpoint } = new URL(authorizationEndpointUrl);
            ...
            frame_ancestor_issue: {
                ...
                const hasIssue = frameAncestorsValues.includes("'none'") || !frameAncestorsValues.includes("'self'");
                if (!hasIssue) {
                    break frame_ancestor_issue;
                }
                return new OidcInitializationError_1.OidcInitializationError({
    ...
    return new OidcInitializationError_1.OidcInitializationError({
        ...
        messageOrCause: [
            ...
            ...(() => {
                ...
                const kc = (0, keycloak_1.createKeycloakUtils)({ issuerUri });
                return [
...
function createInvalidClientCredentialsInitializationError(params) {
