...
exports.tokenSubstitution = tokenSubstitution;
...
function generatePlaceholderForToken(params) {
    ...
    if (match === null) {
        (0, assert_1.assert)(tokenType !== "id_token", "39232932927");
    ...
    const sig_placeholder = (function makeZeroPaddedBase64UrlString(targetLength, seed) {
        const PAD = "A";
...
function getTokensPlaceholders(params) {
...
function substitutePlaceholderByRealToken(text) {
    ...
    (0, assert_1.assert)(typeof text === "string", "394833403");
    ...
    for (const entry of entries) {
        ...
        for (const tokenType of ["idToken", "accessToken", "refreshToken"]) {
            ...
            (0, assert_1.assert)(placeholder !== undefined, "023948092393");
            const realToken = entry.tokens[tokenType];
            (0, assert_1.assert)(realToken !== undefined, "02394809239328");
...
function tokenSubstitution(params) {
    const { trustedExternalResourceServers = [], trustedExternalServiceWorkerSources = [] } = params ?? {};
    const enableTokenSubstitution = () => {
        ...
        earlyInit_prModuleCreateOidc_1.prModuleCreateOidc.then(({ registerExports_tokenSubstitution }) => {
...
function patchFetchApiToSubstituteTokenPlaceholder(params) {
    ...
    //@ts-expect-error
    const fetchLater_actual = window.fetchLater;
    const createFetchOrFetchLater = (params) => {
        ...
        const fn = async (input, init) => {
            ...
            fetch_later: {
                ...
                (0, assert_1.assert)(fetchLater_actual !== undefined);
                const activateAfter = 
                //@ts-expect-error
...
function patchXMLHttpRequestApiToSubstituteTokenPlaceholder(params) {
    ...
    XMLHttpRequest.prototype.open = function open(method, url, ...rest) {
        ...
        return open_actual.call(this, method, state.url, 
        //@ts-expect-error
        ...rest);
    };
    XMLHttpRequest.prototype.setRequestHeader = function setRequestHeader(name, value) {
        const state = stateByInstance.get(this);
        (0, assert_1.assert)(state !== undefined, "29440283");
        const nextValue = substitutePlaceholderByRealToken(value);
        if (nextValue !== value) {
            state.didSubstitute = true;
        }
    ...
    XMLHttpRequest.prototype.send = function send(body) {
        const state = stateByInstance.get(this);
        (0, assert_1.assert)(state !== undefined, "32323484");
        let nextBody = body;
...
function patchWebSocketApiToSubstituteTokenPlaceholder(params) {
    ...
    const WebSocketPatched = function WebSocket(url, protocols) {
        ...
        const instance = new WebSocket_actual(nextUrl, nextProtocols);
    ...
    WebSocket_actual.prototype.send = function send(data) {
        ...
        if (state === undefined && (0, isLikelyDevServer_1.getIsLikelyDevServer)()) {
            return send_actual.call(this, data);
        }
        (0, assert_1.assert)(state !== undefined, "49204832");
        let nextData = data;
...
//# sourceMappingURL=earlyInit_tokenSubstitution.js.map