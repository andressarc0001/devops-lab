...
const generateES256DPoPProof_1 = require("../tools/generateES256DPoPProof");
...
function createAndRecordDPoPStore(params) {
        ...
        case "indexedDB":
            {
                ...
                const prDb = new Promise((resolve, reject) => {
                ...
                const KEY = "dpopState";
                dpopStore = {
                    get: async () => await runTransaction("readonly", store => store.get(KEY)),
    ...
    (0, assert_1.assert)(!(configId in dpopStoreByClientId), "30439");
    dpopStoreByClientId[configId] = dpopStore;
    return dpopStore;
}
async function getDpopState_assertPresent(params) {
    const { configId } = params;
    const dpopStore = dpopStoreByClientId[configId];
    (0, assert_1.assert)(dpopStore !== undefined, "39434");
    const dpopState = await dpopStore.get();
    (0, assert_1.assert)(dpopState, "4933");
    return dpopState;
}
/**
 ...
 */
const exports_createDPoPStore = ({ implementation, configId, clientId }) => {
    const dpopStore = createAndRecordDPoPStore({ implementation, configId });
    function addToJSONPropertyToKeys(dpopState) {
    ...
    return {
        ...
        get: async () => {
            const dpopState = await dpopStore.get();
            (0, assert_1.assert)(dpopState !== undefined, "4806");
...
function generateMaterialToUpgradeBearerRequestToDPoP(params) {
    ...
    const prKeyPair = (async () => {
        ...
        (0, assert_1.assert)(dpopState !== undefined, "304922047");
        return dpopState.keys;
    })();
    const nonceEntries = (nonceEntriesByConfigId[configId] ?? (nonceEntriesByConfigId[configId] = []));
    const origin = new URL(url).origin;
    const generateDPoPProof = async () => (0, generateES256DPoPProof_1.generateES256DPoPProof)({
...
function registerInterceptors() {
        ...
        const createFetchProxy = (params) => {
            ...
            const fetchProxy = async (input, init) => {
                ...
                re_send_with_DPoP_nonce: {
                    ...
                    response = await fetch_ctx(new Request(request_cloneForReplay, {
                        headers: (() => {
    ...
    // our fork of oidc-client-ts.
    {
        ...
        window.fetch = async (input, init) => {
            handle: {
                ...
                const { url, accessToken, httpMethod, keyPair: { configId }, nonce } = JSON.parse(match[1]);
                const dpopState = await getDpopState_assertPresent({ configId });
                const { paramsOfCreateGetServerDateNow } = accessTokenConfigIdEntries
                    .filter(entry => entry.configId === configId)
                    .reverse()
                    .find(() => true) ?? {};
                const dpopProof = await (0, generateES256DPoPProof_1.generateES256DPoPProof)({
                    keyPair: dpopState.keys,
                    url,
                    accessToken,
                    httpMethod,
                    nonce,
                    getServerDateNow: paramsOfCreateGetServerDateNow === undefined
                        ? () => Date.now()
                        : (0, getServerDateNow_1.createGetServerDateNow)(paramsOfCreateGetServerDateNow)
                });
                if (init.headers instanceof Headers) {
    ...
    }
    {
        ...
        XMLHttpRequest.prototype.open = function () {
            ...
            return XMLHttpRequest_prototype_actual.open.call(this, 
            // @ts-expect-error
            ...arguments);
        };
        XMLHttpRequest.prototype.setRequestHeader = function (name, value) {
            intercept: {
                ...
                (0, assert_1.assert)(state !== undefined, "34308330");
                const result = generateMaterialToUpgradeBearerRequestToDPoP({
            ...
            return XMLHttpRequest_prototype_actual.setRequestHeader.call(this, 
            // @ts-expect-error
            ...arguments);
        };
        XMLHttpRequest.prototype.send = function () {
            intercept: {
                ...
                nextStepDPoP().then(({ dpopProof, registerDPoPNonce }) => {
                    ...
                    XMLHttpRequest_prototype_actual.setRequestHeader.call(this, "Authorization", `DPoP ${accessToken}`);
...
function DPoP(params) {
    const { mode } = params;
    const enableDPoP = () => {
        registerInterceptors();
        earlyInit_prModuleCreateOidc_1.prModuleCreateOidc.then(({ registerExports_DPoP }) => {
...
//# sourceMappingURL=earlyInit_DPoP.js.map