import { getStateDataCookies } from "../../core/StateDataCookie.mjs";
export function __withOidcSpaServerEntry(serverEntry_original) {
    return {
        ...
        fetch: async (request, requestOpts) => {
            render_deepLink_instead_of_home_on_authResponse: {
                ...
                const entry = stateDataCookies.find(entry => entry.stateUrlParamValue === stateUrlParamValue);
                if (entry === undefined) {
                    break render_deepLink_instead_of_home_on_authResponse;
                }
                const { stateDataCookie } = entry;
                const rootRelativeRedirectUrl = (() => {
                    if (stateDataCookie.action === "login" &&
...
//# sourceMappingURL=withOidcSpaServerEntry.mjs.map
