...
import { resolveCandidate, resolvePackageFile, normalizeAbsolute, splitId, normalizeRequestPath } from "./utils.mjs";
...
export function createHandleClientEntrypoint(params) {
    ...
    async function load_handleClientEntrypoint(params) {
        ...
        return [
            ...
            `BASE_URL: ${(() => {
                switch (projectType) {
                    case "nuxt": return "__NUXT__.config.app.baseURL";
                    default: return `"${resolvedConfig.base}"`;
                }
            })()},`,
            // prettier-ignore
        ...
        function paramsToJsLiteral_tokenSubstitution(params) {
            const { enabled, trustedExternalResourceServers = [], trustedExternalServiceWorkerSources = [], ...rest } = params;
            assert;
            const mustacheStrToJsTemplateStringLiteral = (mustacheStr) => `\`${mustacheStr.replace(/{{/g, "${").replace(/}}/g, "}")}\``;
            const arrToJsLiteral = (arr) => [
                "[",
            ...
            return [
                ...
                `trustedExternalServiceWorkerSources: ${arrToJsLiteral(trustedExternalServiceWorkerSources)},`,
                "}"
            ].join(" ");
        }
    }
    ...
    switch (projectType) {
        ...
        case "nuxt": {
            ...
            if (typeof rollupInput === "string") {
                ...
                assert(Object.keys(inputRecord).length > 0, "Nuxt rollupOptions.input object must contain at least one entry");
        ...
        case "other": {
            const indexHtmlPath = (() => {
                ...
                const indexHtmlPath = htmlCandidates[0] ?? (existsSync(defaultIndexHtml) ? defaultIndexHtml : undefined);
            ...
            const scriptRegex = /<script\b[^>]*\btype\s*=\s*["']module["'][^>]*\bsrc\s*=\s*["']([^"']+)["'][^>]*>/gi;
            let match;
...
//# sourceMappingURL=handleClientEntrypoint.mjs.map
